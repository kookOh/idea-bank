...
const APPBRAIN_PAGES = [
  { url: 'https://www.appbrain.com/stats/new-popular-android-apps', label: '신규 인기 앱' },
  { url: 'https://www.appbrain.com/stats/google-play-rankings/top_growing/all_categories/kr', label: '급상승 앱' },
];
...
async function fetchAppBrainTrending(): Promise<RawIdea[]> {
  ...
  for (const page of APPBRAIN_PAGES) {
    try {
      ...
      // AppBrain 앱 링크: /app/slug/com.example.app
      ...
      while ((match = appPattern.exec(html)) !== null) {
        ...
        results.push({
          ...
          score: 0,
          comment_count: 0,
          raw_data: { appId, slug, category: page.label, store: 'google_play', via: 'appbrain' },
        });
      }
    } catch (err) {
