...
export async function GET(req: Request) {
  ...
  if (!ideas || ideas.length < 3) {
    return NextResponse.json({ message: 'Not enough data for digest', count: ideas?.length ?? 0 });
  }

  // Groq로 hot_topics + market_insights 생성
  let hot_topics: string[] = [];
  let market_insights = '';

  try {
    const groq = new Groq({ apiKey: process.env.GROQ_API_KEY });
    const ideaSummaries = ideas.map((i, idx) => `${idx + 1}. ${i.title}: ${i.summary_ko ?? ''}`).join('\n');
...
hot_topics는 3-5개의 핵심 트렌드 키워드.`,
  ...
  return NextResponse.json({ date: today, ideas_count: ideas.length, hot_topics_count: hot_topics.length });
}
